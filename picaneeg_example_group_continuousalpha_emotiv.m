function pall=picaneeg_example_group_continuousalpha_emotiv()
...
%  participants
p=picaneeg_example_continuousalpha_emotiv(-1,-1,0,char(group1files(1).name));
...
for ct=1:length(group1files)
  p=picaneeg_example_continuousalpha_emotiv(-1,-1,0,char(group1files(ct).name));
  pall.EEGsummary(sub,:)=p.CondStats.CondMean;
  pall.EEG(sub,:,:)=squeeze(p.CondMeans);
  pall.ipf(sub)=p.alphastats.ipf;
  pall.group(sub)=1;
  sub=sub+1;
end
for ct=1:length(group2files)
  p=picaneeg_example_continuousalpha_emotiv(-1,-1,0,char(group2files(ct).name));
  pall.EEGsummary(sub,:)=p.CondStats.CondMean;
  pall.EEG(sub,:,:)=squeeze(p.CondMeans);
  pall.ipf(sub)=p.alphastats.ipf;
  pall.group(sub)=2;
