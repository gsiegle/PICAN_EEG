function [s] = pican_eeg_read_bdf_big(filename,SecLoad,reshape)
...
% modified for big files by Greg Siegle, gsiegle@pitt.edu
...
s.EEG=zeros(numChan,RecsLoad.*numSam);
%s.EEG = [];
fprintf('Reading %d indices\n',RecsLoad);
for ind = 1:RecsLoad
  if mod(ind,100)==0, fprintf('.'); end
  aux = fread(fid,[RecDur*numSam,numChan],'bit24')';
  if reshape == 'y'
    aux = aux(idx,:);
  end;
  s.EEG(:,1+[(ind-1).*numSam:(ind).*numSam-1])=aux;
end;
fprintf('\n');
fclose(fid);

s.EEG=s.EEG';
%s.EEG=(((s.EEG-Dmin).*gain)+Pmin)';
s.numChan=numChan;
s.labels=labels;
s.txt=txt;
s.fs=fs;
s.gain=gain;
s.prefiltering=prefiltering;
s.ChanDim=ChanDim;
